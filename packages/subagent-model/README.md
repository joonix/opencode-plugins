...
- Extends the `subagent` tool's input schema with two optional string fields and documents them in the tool
  description, so the parent model knows it can pick a child's model and reasoning effort.
...
Append `@<version>` to pin a release. The plugin takes no options. Its plugin id is `joonix.subagent-model`, the id
matched by a `plugins` disable selector such as `-joonix.subagent-model` or `-joonix.*`.
...
Pinned `@<version>` entries and directory entries are left alone. The running service picks up the new version on its
own; restart it only if the change does not appear.
...
A package entry such as `@joonix/opencode-subagent-model` resolves through the `exports` map to `src/index.ts`; a
directory entry resolves `<dir>`, so the `index.ts` file at the package root re-exports `src/` to keep that path
working.
