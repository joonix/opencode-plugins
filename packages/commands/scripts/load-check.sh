...
"$OPENCODE" service set port "$((48500 + RANDOM % 400))" >/dev/null
...
if grep -qE "failed to load plugin\" (plugin.id=joonix\.commands|target=$PLUGIN_DIR)" "$LOG"; then
  grep -E "failed to load plugin\" (plugin.id=joonix\.commands|target=$PLUGIN_DIR)" "$LOG" | tail -1
  exit 1
fi
